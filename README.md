...
1. When using Git locally, what are these?  Define each one in a sentence
   * Staging area - where changes made to files are prepared and organized before they are committed
   * Working copy - where to make modifications to the files
   * master - the primary branch of a repository
   * HEAD - the latest commit in the currently checked-out branch
...
3. There are 2 ways to create a local Git repository.  Briefly descibe each one:
   - Create a new local Git repo in the current directory
     - use ```git init``` to create a .git directory in your current directory then you can commit files into the repository
   - Clone a remote repo to your current directory
     - use ```git clone <url>``` to create a local copy of an existing remote Git repository
...
1. Create new branch and move HEAD to it
   ```
   git checkout -b <branchname>
   ```
2. Reverse changes and share those reversed changes with others.
   ```
   git revert
   ```
3. Merging a branch
   ...
   git merge
   ```
4. Delete a branch after the changes are merged and you no longer need the branch
   ```
...
- I want to add many files to the stage area but there is a file name *a.py* that I don't want it in the stage area
   ```
  git add -all
  git rm a.py
  ```
...
* [Github Basics](https://docs.github.com/en/get-started/using-git) the Basic commands of git.
